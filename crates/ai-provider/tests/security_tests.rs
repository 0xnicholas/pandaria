...
fn test_provider_error_no_raw_body() {
    ...
    let err = ai_provider::LlmError::ProviderError(
...
fn test_provider_error_extracts_json_message() {
    ...
    let msg = ai_provider::http_error::sanitize_http_error_body(400, body);
...
fn test_provider_error_sanitizes_html() {
    ...
    let msg = ai_provider::http_error::sanitize_http_error_body(502, body);
...
fn test_api_key_not_in_error_display() {
    ...
    let err = ai_provider::LlmError::AuthError("OPENAI_API_KEY not set".to_string());
    let display = format!("{}", err);
