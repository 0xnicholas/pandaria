...
mod tests {
    ...
    #[test]
    fn test_heading() {
        assert!(!render_markdown("# Hello", &Theme::default(), "base16-ocean.dark").is_empty());
    }
    #[test]
    fn test_code_block() {
        assert!(
    ...
    #[test]
    fn test_paragraph() {
        let r = render_markdown("hello world", &Theme::default(), "base16-ocean.dark");
        assert!(r[0].spans.iter().any(|s| s.content.contains("hello")));
    }
    ...
    fn test_inline_code() {
        ...
        assert!(
            r.iter()
                .any(|l| l.spans.iter().any(|s| s.content.contains("Vec::new()")))
        );
    }
    ...
    fn test_italic_text() {
        ...
        assert!(
            r.iter()
                .any(|l| l.spans.iter().any(|s| s.content.contains("italic")
                    && s.style.add_modifier == ratatui::style::Modifier::ITALIC))
        );
    }
    ...
    fn test_nested_bold_italic() {
        ...
        let has_both = r
            .iter()
            .any(|l| l.spans.iter().any(|s| s.content.contains("both")));
        assert!(has_both);
    }
    ...
    fn test_multiple_paragraphs() {
        ...
        assert!(
            r.iter()
                .any(|l| l.spans.iter().any(|s| s.content.contains("p1")))
        );
        assert!(
            r.iter()
                .any(|l| l.spans.iter().any(|s| s.content.contains("p2")))
        );
    }
}
