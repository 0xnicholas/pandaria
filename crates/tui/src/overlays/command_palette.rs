...
impl CommandPalette {
    pub fn new() -> Self { Self { filter: String::new(), selected: 0, confirmed: None, dismissed: false } }
...
impl Component for CommandPalette {
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        let overlay_area = Rect::new(area.x + (area.width.saturating_sub(50)) / 2, area.y + (area.height.saturating_sub(h)) / 3, 50, h);
    ...
    fn handle_input(&mut self, key: KeyEvent) -> InputResult {
        match key.code {
            KeyCode::Char(c) => { self.filter.push(c); self.selected = 0; InputResult::Consumed }
            KeyCode::Backspace => { self.filter.pop(); self.selected = 0; InputResult::Consumed }
            KeyCode::Up => { if self.selected > 0 { self.selected -= 1; } InputResult::Consumed }
            KeyCode::Down => { let n = self.filtered().len(); if n > 0 && self.selected + 1 < n { self.selected += 1; } InputResult::Consumed }
            KeyCode::Enter => {
                let f = self.filtered();
...
mod tests {
    ...
    fn test_enter_confirms_selection() {
        ...
        let result = cp.handle_input(KeyEvent::new(KeyCode::Enter, crossterm::event::KeyModifiers::NONE));
    ...
    fn test_esc_dismisses() {
        let mut cp = CommandPalette::new();
        cp.handle_input(KeyEvent::new(KeyCode::Esc, crossterm::event::KeyModifiers::NONE));
    ...
    fn test_down_arrow_navigates() {
        let mut cp = CommandPalette::new();
        cp.handle_input(KeyEvent::new(KeyCode::Down, crossterm::event::KeyModifiers::NONE));
        cp.handle_input(KeyEvent::new(KeyCode::Down, crossterm::event::KeyModifiers::NONE));
        let result = cp.handle_input(KeyEvent::new(KeyCode::Enter, crossterm::event::KeyModifiers::NONE));
        assert!(matches!(result, InputResult::Consumed));
    }
