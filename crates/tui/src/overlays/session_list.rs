...
impl SessionListOverlay {
    pub fn new(sessions: Vec<(String, String)>) -> Self { Self { sessions, selected: 0, confirmed: None, dismissed: false } }
}
...
impl Component for SessionListOverlay {
    ...
    fn handle_input(&mut self, key: KeyEvent) -> InputResult {
        match key.code {
            ...
            KeyCode::Down | KeyCode::Char('j') => { if self.selected + 1 < self.sessions.len() { self.selected += 1; } InputResult::Consumed }
            KeyCode::Enter => { self.confirmed = self.sessions.get(self.selected).map(|(id, _)| id.clone()); InputResult::Consumed }
            KeyCode::Esc => { self.dismissed = true; InputResult::Consumed }
            KeyCode::Delete | KeyCode::Char('d') => { self.confirmed = self.sessions.get(self.selected).map(|(id, _)| format!("delete:{}", id)); InputResult::Consumed }
...
mod tests {
    ...
    fn test_j_down_navigates() {
        ...
        sl.handle_input(KeyEvent::new(KeyCode::Char('j'), crossterm::event::KeyModifiers::NONE));
    ...
    fn test_enter_confirms_selected() {
        let mut sl = make_list();
        sl.selected = 1;
        sl.handle_input(KeyEvent::new(KeyCode::Enter, crossterm::event::KeyModifiers::NONE));
    ...
    fn test_esc_dismisses() {
        let mut sl = make_list();
        sl.handle_input(KeyEvent::new(KeyCode::Esc, crossterm::event::KeyModifiers::NONE));
    ...
    fn test_d_sends_delete() {
        let mut sl = make_list();
        sl.selected = 0;
        sl.handle_input(KeyEvent::new(KeyCode::Char('d'), crossterm::event::KeyModifiers::NONE));
        assert_eq!(sl.take_result(), OverlayResult::Confirmed("delete:s1".into()));
    }
}
