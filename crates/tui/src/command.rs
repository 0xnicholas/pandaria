...
pub enum Command {
    ...
    Retry,
    Copy,
    Dump { filename: Option<String> },
    Compact,
    Rename { title: String },
    Tree,
    Fork { message_id: Option<String> },
    Settings,
...
impl Command {
    pub fn parse(input: &str) -> Option<Self> {
        ...
        match cmd {
            ...
            "switch" if !args.is_empty() => Some(Command::SwitchSession {
                id: args.to_string(),
            }),
            "list" => Some(Command::ListSessions),
            "model" => Some(Command::SelectModel {
                id: if args.is_empty() {
                    None
                } else {
                    Some(args.to_string())
            ...
            "connect" if !args.is_empty() => Some(Command::Connect {
                url: args.to_string(),
            }),
            "auth" if !args.is_empty() => Some(Command::Auth {
            ...
            "rename" if !args.is_empty() => Some(Command::Rename {
                title: args.to_string(),
            }),
            "tree" => Some(Command::Tree),
            "fork" => Some(Command::Fork {
                message_id: if args.is_empty() {
                    None
                } else {
                    Some(args.to_string())
                },
            ...
            "import" if !args.is_empty() => Some(Command::Import {
                filename: args.to_string(),
            }),
            "delete" => Some(Command::DeleteSession),
            "system" if !args.is_empty() => Some(Command::SystemPrompt {
                prompt: args.to_string(),
            }),
            "skill" if !args.is_empty() => Some(Command::Skill {
                name: args.to_string(),
            }),
            _ => {
...
mod tests {
    ...
    fn test_switch() {
        assert_eq!(
            Command::parse("/switch abc"),
            Some(Command::SwitchSession { id: "abc".into() })
        );
    }
    #[test]
    fn test_not_command() {
    ...
    #[test]
    fn test_retry() {
        assert_eq!(Command::parse("/retry"), Some(Command::Retry));
    }
    #[test]
    fn test_copy() {
    ...
    fn test_dump() {
        assert_eq!(
            Command::parse("/dump out.md"),
            Some(Command::Dump {
                filename: Some("out.md".into())
            })
        );
    }
    #[test]
    fn test_compact() {
        assert_eq!(Command::parse("/compact"), Some(Command::Compact));
    }
    #[test]
    fn test_rename() {
        assert_eq!(
            Command::parse("/rename my session"),
            Some(Command::Rename {
