...
impl AnthropicProvider {
    ...
    pub fn new(api_key: Option<SecretString>) -> Self {
        Self::with_base_url(api_key, "https://api.anthropic.com/v1/messages")
    }
    ...
    fn resolve_api_key(
        ...
        Err(LlmError::AuthError("ANTHROPIC_API_KEY not set".to_string()))
    }
}
    ...
    async fn stream(
        ...
        tokio::spawn(async move {
            let result = Self::try_stream(
                client, base_url, &model, context, options, &tx, api_key, signal,
            )
            .await;
            if let Err(e) = result {
                let _ = tx
