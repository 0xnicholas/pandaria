# llm-client v0.2 开发实施计划

> **For agentic workers:** REQUIRED SUB-SKILL: Use `superpowers:subagent-driven-development` or `superpowers:executing-plans` to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 将 llm-client crate 从 v0.1（核心功能完成）升级到完整 spec 覆盖，补全 wiremock 集成测试、P3 Provider（Mistral/Bedrock）、OAuth 抽象层，并完成最终质量验证。
...
**联合开发时序:**
```
全程并行: llm-client v0.2 P1 (Phase 1) 
          └─ 与 agent-core Phase 0-9 + extensions Phase 1-4 完全独立，可同时执行
可选并行: llm-client v0.2 P3 (Phase 2-3)
...
**Files:**
- Create: `crates/llm-client/tests/anthropic_tests.rs`
- Create: `crates/llm-client/tests/openai_tests.rs`
- Create: `crates/llm-client/tests/google_tests.rs`
...
```bash
cargo test -p llm-client --test anthropic_tests --test openai_tests --test google_tests
```
...
```bash
git add crates/llm-client/tests/anthropic_tests.rs crates/llm-client/tests/openai_tests.rs crates/llm-client/tests/google_tests.rs
git commit -m "test(llm-client): add wiremock HTTP mock tests for providers"
```
...
**Files:**
- Create: `crates/llm-client/tests/types_serde.rs`
- Create: `crates/llm-client/tests/streaming_tests.rs`
- Create: `crates/llm-client/tests/retry_tests.rs`
- Create: `crates/llm-client/tests/overflow_tests.rs`
- Create: `crates/llm-client/tests/validation_tests.rs`
- Create: `crates/llm-client/tests/models_tests.rs`
- Create: `crates/llm-client/tests/compat_tests.rs`
- Create: `crates/llm-client/tests/repair_tests.rs`
- Create: `crates/llm-client/tests/transform_tests.rs`
- Create: `crates/llm-client/tests/security_tests.rs`
...
```bash
git add crates/llm-client/tests/
git commit -m "test(llm-client): migrate module tests to tests/ directory"
```
...
**Files:**
- Modify: `crates/llm-client/src/providers/openai.rs:250-350`（reasoning 参数构建区域）
...
```bash
git add crates/llm-client/src/providers/openai.rs
git commit -m "feat(llm-client): add thinking_format multi-provider mapping for OpenAI"
```
...
**Files:**
- Modify: `crates/llm-client/README.md`
- Modify: `crates/llm-client/src/transform.rs:1`（修复 unused import warning）

- [ ] **Step 1.4.1: 修复 compiler warning**

```rust
// crates/llm-client/src/transform.rs:1
...
在 `crates/llm-client/README.md` 中：
...
```bash
git add crates/llm-client/README.md crates/llm-client/src/transform.rs
git commit -m "docs(llm-client): update README and fix warnings"
```
...
**Files:**
- Create: `crates/llm-client/src/providers/mistral.rs`
- Modify: `crates/llm-client/src/providers/mod.rs`
- Modify: `crates/llm-client/src/lib.rs`
...
```rust
// crates/llm-client/src/providers/mod.rs
pub mod anthropic;
pub mod google;
pub mod mistral;
pub mod openai;

// crates/llm-client/src/lib.rs
...
```bash
git add crates/llm-client/src/providers/mistral.rs crates/llm-client/src/providers/mod.rs crates/llm-client/src/lib.rs
git commit -m "feat(llm-client): add MistralProvider"
```
...
**Files:**
- Create: `crates/llm-client/src/providers/bedrock.rs`
- Modify: `crates/llm-client/src/providers/mod.rs`
- Modify: `crates/llm-client/src/lib.rs`
- Modify: `crates/llm-client/Cargo.toml`
...
```bash
cargo test -p llm-client --features bedrock
cargo build -p llm-client --all-features
```

- [ ] **Step 2.2.10: Commit**

```bash
git add crates/llm-client/src/providers/bedrock.rs crates/llm-client/src/providers/mod.rs crates/llm-client/src/lib.rs crates/llm-client/Cargo.toml
git commit -m "feat(llm-client): add AwsBedrockProvider (optional feature)"
```
...
**Files:**
- Create: `crates/llm-client/src/oauth.rs`
- Modify: `crates/llm-client/src/lib.rs`
...
```bash
git add crates/llm-client/src/oauth.rs crates/llm-client/src/lib.rs crates/llm-client/src/providers/anthropic.rs crates/llm-client/src/providers/openai.rs crates/llm-client/src/providers/google.rs crates/llm-client/tests/oauth_tests.rs
git commit -m "feat(llm-client): add OAuth abstraction layer and integrate with providers"
```
...
# 全量测试
cargo test -p llm-client --all-features
```

### 预期最终状态：

- [ ] llm-client: ~150+ 测试全部通过
...
- **v0.1 计划** (`2026-05-02-llm-client-implementation.md`): 已完成 ✅
