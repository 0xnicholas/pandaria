...
| tavern（Agent Team 编排层） | ✅ 核心组件已实现。旧 WorkflowEngine/AgentRuntime/tavern-flow-macros 已删除。SquadEngine（DAG + Hierarchical）+ PandariaAgentExecutor（agent-core 桥接）+ EventStore（PG/SQLite/Memory）+ replay + Webhook。Hero 保留为 AgentResolver。 |
